...
import { useAppSelector, useAppDispatch } from "../../library/common/hooks/hooks";
...
const ZeroBoard = () => {
   ...
   function addCol() {
      const boardsUpdated = boards.map(board => board.id !== activeBoardId ? board : {
         ...board,
         columns: [{id: 0, color: COLUMNCOLORS[0], name: 'NEWCOLUMN1', tasks: []}]
      })
   ...
   return (
      <Box sx={{height: '80vh', width: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
         <Typography sx={{fontSize: '18px', color: '#828FA3', fontWeight: 700, mb: 4, textAlign: 'center'}}>
